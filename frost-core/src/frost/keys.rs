    ...
    C: Ciphersuite,
{
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // If Scalar<C> implements Debug, directly use self.scalar
        f.debug_struct("SigningShare")
         .field("scalar", &hex::encode(self.serialize()))
         .finish()
    }
}
...
// impl<C> Debug for Scalar<C>
// where
//   C: Ciphersuite,
// {
//     fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
//         // You would need to have a way to convert Scalar to a debuggable format
//         write!(f, "Scalar({})", self.to_debug_string())  // Assuming such a method exists
//     }
// }
