...
fn check_serialize_vss_commitment() {
    let rng = thread_rng();
    frost_core::tests::vss_commitment::check_serialize_vss_commitment::<BabyJubJubSha256, _>(rng);
}
...
fn check_deserialize_vss_commitment_error() {
    let rng = thread_rng();
    frost_core::tests::vss_commitment::check_deserialize_vss_commitment_error::<BabyJubJubSha256, _>(
        rng, &ELEMENTS,
    );
}
